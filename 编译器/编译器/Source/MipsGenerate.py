...
def PrintTable():
    ...
    for i in range(len(DFA.state)):
        ...
        for j in range(len(LeftGroup)):
            row.append(GOTO[i][j])
        x.append(row)
    with open('../../Source/lr(1).table','w') as fd:
        for row in x:
            for colum in row:
                fd.write(colum+'\t')
            fd.write('\n')
...
def MakeAnalyse():
    ...
    while True:
        ...
        if Action=='acc':
            ...
            with open('../../Source/analysis.table', 'w') as fd:
                for row in Table:
                    for colum in row:
                        fd.write(colum + '\t')
                    fd.write('\n')
...
def PrintIntermediateCode(codes):
    fd=open('../../Source/result.middle','w')
    for code in codes:
        if code[0]==':=':
...
def SyntaxParserTable():
    ReadGrammer("../../Source/grammer.txt")
...
def MipsGenerate():
    ...
    GenerateMips(codes)
    with open('../../Source/result.asm','w') as f:
        for code in Mips:
            if code == 'main:':
                f.write('\n')
            f.write(code+'\n')
