        ...
        for item in ProductionGroup:
            if(item[0]==NonTerminal):
                ...
                for RightExpression in RightExpressionGroup:#每一句单独分析
                    ...
                    cnt=len(SplitResult)
                    #终结符在句首或者句尾
                    if (SplitResult[0].keys()[0] == "Terminal" and SplitResult[0].values()[0] not in FirstDic['value']):
                        FirstDic['value'].append(SplitResult[0].values()[0])
                    if (SplitResult[cnt-1].keys()[0] == "Terminal" and SplitResult[cnt-1].values()[0] not in LastDic['value']):
                        LastDic['value'].append(SplitResult[cnt-1].values()[0])
                    # 非终结符在句首或者句尾
                    if (SplitResult[0].keys()[0] == "Nonterminal"):
                        if(SplitResult[0].values()[0] not in FirstDic['value']):
                            FirstDic['value'].append(SplitResult[0].values()[0])#后续添加该非终结符的FIRSTVT
                        if(cnt>1 and SplitResult[1].keys()[0] == "Terminal"and SplitResult[1].values()[0] not in FirstDic['value']):
                            FirstDic['value'].append(SplitResult[1].values()[0])#添加第二个终结符到FIRSTVT
                    if (SplitResult[cnt-1].keys()[0] == "Nonterminal"):
                        if(SplitResult[cnt-1].values()[0] not in LastDic['value']):
                            LastDic['value'].append(SplitResult[cnt-1].values()[0])#后续添加该非终结符的LASTVT
                        if(cnt>1 and SplitResult[cnt-2].keys()[0] == "Terminal" and SplitResult[cnt-2].values()[0] not in LastDic['value']):
                            LastDic['value'].append(SplitResult[cnt - 2].values()[0])#添加倒数第二个终结符到LASTVT
...
def MakeUpFIRSTVT(FirstDic):
    TempDic=FirstDic
    for item in FirstDic['value']:
        if item in NonterminalSymbolGroup:
            TempDic['value'].remove(item)
            for tuple in FIRSTVT:
                if(tuple['item']==item):
                    temp=MakeUpFIRSTVT(tuple)
                    for terminal in temp['value']:
                        if(terminal not in FirstDic['value']):
                            TempDic['value'].append(terminal)
        else:
            continue
    return TempDic
def MakeUpLASTVT(LastDic):
    TempDic=LastDic
    for item in LastDic['value']:
        if item in NonterminalSymbolGroup:
            TempDic['value'].remove(item)
            for tuple in LASTVT:
                if(tuple['item']==item):
                    ...
                    for terminal in temp['value']:
                        if(terminal not in LastDic['value']):
                            TempDic['value'].append(terminal)
        else:
            continue
    return TempDic
