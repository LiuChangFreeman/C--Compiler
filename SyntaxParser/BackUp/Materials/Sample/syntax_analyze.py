...
class SyntaxAnalyze(object):
    ...
    def create_lr_dfa(self):
        ...
        def expand_production(self, cur_production, ex_object_set):
            ...
            position = cur_production[3]
            terminal_tail = cur_production[4]
            if position < len(right) and (right[position] in self.noterminate):
                for production in self.productions_dict[right[position]]:
                    new_tail_set = set()
                    flag = True
                    for i in range(position + 1, len(right)):
                        ...
                        if '$' in cur_first_set:
                            ...
                            break
                    if flag:
                        new_tail_set = tuple(set(new_tail_set) | set(terminal_tail))
                    ex_new_production = (
                        production[1],
                        right[position], production[0], 0, new_tail_set)
        ...
        object_set = expand_production(self, (0, 'S', ('start',), 0, '#'), set())
        def ProductionToString(key):
            result = key[1] + '->'
            position = 0
            for data in key[2]:
                if position == key[3]:
        ...
        while object_set_queue:
            ...
            # temp=[ProductionToString(p) for p in old_set]
            # temp.sort()
            # print('\n'.join(temp) + '\n')
            for cur_production in old_set:
                ...
                if point_index >= len(right) or '$' in right:
                    ...
                    if right[point_index] in self.terminate:
                        ...
                        temp=(old_set_id,tar_set_id,right[point_index])
                        GraphView.add_edge(temp)
        ...
        for t in self.terminate:
            title.append(t)
        for nt in self.noterminate:
            title.append(nt)
        title.sort()
        x = PrettyTable(title)
        table=self.lr_analyze_table
        for key in table.keys():
            row = table[key]
            table_row=[key]
            for value in title[1:]:
                if value in row.keys():
                    table_row.append(str(row[value][0])+str(row[value][1]))
                else:
                    table_row.append("")
            x.add_row(table_row)
        print(x)
...
def main():
    ...
    # for j in syn_ana.lr_analyze_table:
    #     print j, ': ', syn_ana.lr_analyze_table[j]
    # for pro in syn_ana.productions:
    #     print syn_ana.productions.index(pro), pro
    # for j in syn_ana.first_set.keys():
    #     print 'j =', j, '\n', 'first =', syn_ana.first_set[j]
