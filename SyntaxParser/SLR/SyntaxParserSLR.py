    ...
    for P in Productions:
        ...
        if RightExpression==[EndSymbol] and EndSymbol not in result:
            ...
            if RightExpression[0]['class']=='T' and RightExpression[0] not in result:
                ...
                if RightExpression[0]['type']!=Symbol:
            ...
            if cnt>1:
                ...
                for i in range(1,cnt):
                    if previous['type']!=Symbol:
                        if not EndSymbol in First(previous['type']):
                            break
                        else:
                            if RightExpression[i]['type']!=Symbol:
                                TempFirst = First(RightExpression[i]['type'])
                                if TempFirst not in result:
                                    result.append(TempFirst)
                ...
                if i==cnt-1:
                    if production.NonterminalSymbol!=Symbol:
                        ...
                        if production.NonterminalSymbol == 'S':
                            ...
                            temp={'class': 'NT', 'type': production.NonterminalSymbol}
                    ...
                    if next['class']=='T':
                        ...
                        temp=FIRST
                        for item in FIRST[next['type']]:
                            if item['type']!='$' and item not in result:
                                result.append(item)
                    if i==cnt-2 and EndSymbol in First(next['type']) and next not in result:
                        result.append(next)
...
def MakeUpFollow():
    ...
    while len(procession)>0:
        ...
        for key in procession:
            ...
            for item in follow:
                if item['class'] == 'NT':
                    if IsFollowComplete(item['type']):
                        for value in FOLLOW[item['type']]:
                            if value not in follow:
                                follow.append(value)
                        follow.remove(item)
                        change =True
