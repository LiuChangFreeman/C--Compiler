...
def PrintTable():#将LR(1)分析表写入文件
    ...
    for i in range(len(DFA.state)):
        ...
        for j in range(len(LeftGroup)):
            row.append(GOTO[i][j])
        x.append(row)
    with open(path+'lr1.table','w') as fd:
        for row in x:
            for colum in row:
                fd.write(colum+'\t')
            fd.write('\n')
...
def MakeAnalyse():#进行语法分析
    ...
    while True:
        ...
        if Action=='acc':
            ...
            with open(path+'analysis.table', 'w') as fd:
                for row in Table:
                    for colum in row:
                        fd.write(colum + '\t')
                    fd.write('\n')
...
if __name__=='__main__':
    ...
    GenerateMips(codes)
    with open(path+'result.asm','w') as f:
        for code in Mips:
            if code == 'main:':
                f.write('\n')
            f.write(code+'\n')
